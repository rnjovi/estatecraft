...
def count_type_in_all_locations(type_keyword):
    query = "SELECT COUNT(*) FROM apartments WHERE type LIKE %s;"
    result = db.fetch_one(query, (f"%{type_keyword}%",))
    return result[0]


def create_layout_query(conditional: str = ""):
    query = f'''
    SELECT
        CASE
...
def count_layouts_by_type_and_location(type_keyword, location_keyword):
    query = create_layout_query("WHERE type ILIKE %s AND address ILIKE %s")
    results = db.fetch(query, (f"%{type_keyword}%", f"%{location_keyword}%"))

    return create_layout_info_list(results)


def count_layouts_by_type(type_keyword):
    query = create_layout_query("WHERE type ILIKE %s")
    results = db.fetch(query, (f"%{type_keyword}%",))

    return create_layout_info_list(results)


def create_layout_info_list(results):
    return [{
        'layout': row[0],
        'count': row[1],
        'avg_price': row[2],
...
def run_queries(type_keyword, location_keyword):
    ...
    layout_infos = count_layouts_by_type_and_location(type_keyword, location_keyword)

    print_layout_info(layout_infos)

    count = count_type_in_all_locations(type_keyword)
    print()
    print(f"There are {count} apartments of type '{type_keyword}' for sale in Finland.")
    layout_infos = count_layouts_by_type(type_keyword)

    print_layout_info(layout_infos)


def print_layout_info(layout_infos):
